...
void sauvegarder_joueur(Joueur *j) {
    ...
    if (pf == NULL) {
        ...
        if (pf == NULL) {
            allegro_message("Erreur lors de la création du fichier joueur.txt");
            exit(1);
        }
        fprintf(pf, "%s %d %d %d\n", j->nom, j->niveau, j->reprise_x, j->reprise_y);
        fclose(pf);
        return;
    }
    ...
    pf = fopen("joueur.txt", "w");
    if (pf == NULL) {
        allegro_message("Erreur d'écriture fichier joueur.txt");
        exit(1);
    }
...
Joueur* nouveau_joueur(BITMAP *screen) {
    ...
    do {
        ...
        if (recherche_dans_fichier(j)) {
            ...
            char msg[100];
            sprintf(msg, "Le pseudo %s existe deja", j->nom);
